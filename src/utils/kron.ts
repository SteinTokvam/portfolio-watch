import { Holding, KronDevelopment, KronInterval, KronValue, Transaction } from "../types"
...
export async function fetchTotalValue(accessKey: string, account_id: string, interval: KronInterval): Promise<KronValue> {
    return await getDevelopment(accessKey, account_id, interval)
    .then(response => {
        const ret = response.data.series.pop()
        return ret ? ret : {date: new Date().toISOString(), market_value: 0, yield: 0, return: 0}
    })
};

export async function fetchHoldings(accessKey: string, accountKey: number, account_id: string): Promise<Holding[]> {
